...
if( !function_exists( 'yit_get_attachment_id' ) ) {
    ...
    function yit_get_attachment_id( $url ) {
        ...
        foreach( $ids as $id ){
            ...
            if( $url == array_shift( $attachment_image ) )
                return $id;
        }
...
if( !function_exists( 'yit_get_standard_shop_sidebar' ) ) {
    ...
    function yit_get_standard_shop_sidebar() {
    	$page = '';
        if( function_exists( 'is_woocommerce' ) && !is_woocommerce() && !is_internal() ) {            
...
if( !function_exists( 'yit_breadcrumb' ) ) {
    ...
    function yit_breadcrumb( $delimiter = '&raquo;' ) {
        ...
        $homeLink = site_url();
        if( !is_home() && !is_front_page() && apply_filters( 'yit_show_breadcrumb_home_link', true ) )
...
//                 echo '<a href="' . $homeLink . '/' . $slug['slug'] . '/">' . $post_type->labels->singular_name . '</a> ' . $delimiter . ' ';
        ...
        } elseif ( !is_single() && !is_page() && get_post_type() != 'post' && get_post_type() != '' && !is_404() ) {
            $post_type = get_post_type_object( get_post_type() );
...
if( !function_exists( 'is_shop_installed' ) ) {
    ...
    function is_shop_installed() {
        ...
        if( is_plugin_active( $woocommerce ) || is_plugin_active( $jigoshop ) ) {
            return true;
        } else {
            return false;
        }
