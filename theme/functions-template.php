...
if( !function_exists( 'yit_add_custom_styles' ) ) {
    function yit_add_custom_styles() {        
        ...
        if( file_exists( YIT_CACHE_DIR . $cache_file ) )
            { yit_wp_enqueue_style( 9999, 'cache-custom', YIT_CACHE_URL . $cache_file, array(), false, 'all', true ); }
        yit_wp_enqueue_style( 99999, 'custom', get_template_directory_uri() . '/custom.css', array(), false, 'all', true );
    }
}
...
if( !function_exists( 'yit_header_background' ) ) {
    ...
    function yit_header_background() {
        ...
        if ( empty( $color ) ) {
            $color = isset( $background['color'] ) ? $background['color'] : $background;
        }
        if ( empty( $image ) ) {
