...
class YIT_Css {
    ...
    public function save_css() {
        ...
        yit_file_put_contents( yit_get_model( 'cache' )->locate_file( str_replace( '.css', $index . '.css', $this->_customFilename ) ), $css );
    }
		...
		foreach( $this->_stylesheets as $s ) {
			...
			if( $type == 'external' ) {
				...
				if( !$exclude && ($media == 'all' || $media == 'screen') ) {
					//wp_enqueue_style( $handle, $src, $deps, $ver, $media );
					$filename = locate_template( str_replace( get_template_directory_uri() . '/', '', $src), false );
					$style = "";
